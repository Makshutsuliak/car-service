  ...
  return (
    <div className="bg-gray-50 min-h-screen p-4 sm:p-6">
      <div className="max-w-5xl mx-auto bg-white shadow-lg rounded-lg p-4 sm:p-6">
        <h2 className="text-2xl sm:text-3xl font-bold mb-6 text-center">Гарантійні талони</h2>
        ...
        {!selectedCar && cars.length > 0 && (
          <div className="mt-4">
            <h3 className="text-lg sm:text-xl font-semibold mb-3">Знайдені авто</h3>
            <ul className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {cars.map((car) => (
                <li
                  key={car.id}
                  onClick={() => fetchCarServices(car.id)}
                  className="border p-3 rounded cursor-pointer hover:bg-gray-100 shadow-sm min-h-[80px] hover:shadow-xl transition-shadow duration-300 text-sm sm:text-base"
                >
        ...
        {selectedCar && !selectedService && (
          <div className="mt-6">
            <h3 className="text-lg sm:text-xl font-semibold mb-3">Послуги для авто</h3>
            {services.length === 0 ? (
              ...
              <ul className="grid grid-cols-1 gap-4">
                {services.map((service) => (
                  <li
                    ...
                    className="border p-3 rounded cursor-pointer hover:bg-gray-100 shadow-sm hover:shadow-xl transition-shadow duration-300 text-sm sm:text-base"
                  >
            ...
            <div className="mt-4 text-center">
              <Button
                ...
                className="bg-gray-600 text-white px-4 py-2 rounded text-sm sm:text-base"
              >
  ...
  >
    ...
    <div className="mt-16 sm:mt-20 flex justify-between items-end">
      ...
      <div className="text-right">
        ...
        <div className="border m-3 w-16 h-16 sm:w-20 sm:h-20 md:w-24 md:h-24 inline-block"></div>
      </div>
    </div>
  </div>
)}
       ...
       {selectedService && (
          <div className="mt-6 flex flex-col sm:flex-row justify-center gap-4">
            <button
              ...
              className="bg-gray-600 text-white px-6 py-2 rounded shadow-md hover:bg-gray-700 transition text-sm sm:text-base"
            >
              Назад до послуг
            </button>
            <button
              onClick={handleDownloadPDF}
              className="bg-green-600 text-white px-6 py-2 rounded shadow-md hover:bg-green-700 transition text-sm sm:text-base"
            >
              Завантажити PDF
            </button>
          </div>
